...
enum enDateCheckTool {Befor = -1,Equal = 0,After = 1};
...
{
    if (CheckDateTool(Period1.EndDate,Period2.StartDate) 
    == enDateCheckTool::After || CheckDateTool(Period1.StartDate,Period2.EndDate)
    == enDateCheckTool::Befor)
        return false;
    return true;
}
...
{
    return !(CheckDateTool(Date,Period.StartDate) 
    == enDateCheckTool::After || CheckDateTool(Date,Period.EndDate)
    == enDateCheckTool::Befor);
}
...
int main()
{
    ...
    cout << '\n' << (CheckDateInPeriod(Date,Period) ? "Yes, " : "No, ") << "Date is within period.";
    return 0;
}